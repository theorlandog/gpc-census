...
def test_generator_named_in_the_artifact_is_absent():
    """Guard the doc's 'not delivered' claim against going stale silently."""
    assert artifact()["generated_by"] == "scripts/fixed_1rdm_observable_ranges.py"
    assert not (ROOT / "scripts" / "fixed_1rdm_observable_ranges.py").exists()
