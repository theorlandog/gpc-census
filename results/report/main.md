...
**The library of states.** Beyond the verdicts, we release explicit extremal states across the census. Of the $799$ vertices spanning ranks $6$--$10$, **$699$ now carry a certified closed-form extremal state**: $631$ design vertices built directly from their witnesses ($\psi = \sum_T \sqrt{k_T/D}\,|T\rangle$, diagonal 1-RDM equal to the spectrum, exact by construction) and $68$ interference or real-design vertices solved by the engine of Section [-@sec:methods] (the $2\times2$ block mechanism of $\psi_B$ and its $k$-clique generalization, producing algebraic amplitudes and phases, each accepted only after the exact 1-RDM identity verifies it). $v_A$ and $v_B$ are the flagship hand-checked instances (Theorems [1](#thm:psiA), [3](#thm:psiB)). The remaining $100$ vertices are a labeled residual of interference corners the current ansatz family does not yet reach. The accompanying machine-readable bundle records, for every vertex, its classification and, where certified, the closed-form state (natural denominator, integer weights, symbolic amplitudes) with its certificate. Before this work, explicit states were published only for the small ($d\le 8$) systems; to our knowledge this is the first systematic library of exact extremal states across the higher-rank generalized Pauli polytopes.
