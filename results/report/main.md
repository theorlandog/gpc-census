...
3.  **A certified atlas of closed-form extremal states through rank 10.** Beyond $v_A$ and $v_B$, we construct and certify explicit closed-form extremal states for the vertices of every determinate system through rank 10: each design vertex directly from its witness (exact by construction), and interference vertices by an exact block/clique phase solve verified in exact arithmetic (the exact 1-RDM identity). The resulting atlas is machine-readable, carries a per-state certificate, and to our knowledge is the first systematic library of exact extremal states for the generalized Pauli polytopes; it accompanies this paper (Section [-@sec:census910]).
...
The atlas of this paper is complete through rank 10, and its extension is an obvious progression rather than a new idea: the same engine constructs and certifies states at any rank, bounded only by compute, so a residual interference vertex is closed by a longer block search on more hardware, not by new mathematics. The one genuinely missing piece beyond rank 10 is the constraint list itself, and supplying a generator for it is the subject of forthcoming work. The gap this paper addresses at rank $9$ is an instance of a larger one: constraint tables exist only for $d \le 10$ (with $(3,d)$ systems reported to $d = 12$), while quantum-chemical applications need $d \sim 20$--$40$; the polytope program has largely paused at this frontier, in part because extremal-state construction has been manual. We outline a four-instrument program toward that regime, distinguishing carefully between rigorous and heuristic layers.
...
**The atlas of states.** Beyond the verdicts, we release an explicit extremal state for each vertex. Every design vertex (the large majority, $85$ of $103$ at $\wedge^4\mathcal{H}_9$ and correspondingly through rank $10$) has a closed form by construction, $\psi = \sum_T \sqrt{k_T/D}\,|T\rangle$ on its witness support, with a diagonal 1-RDM equal to the spectrum. The interference vertices are handled by the engine of Section [-@sec:methods]: the $2\times2$ block mechanism of $\psi_B$ and its $k$-clique generalization produce closed forms with algebraic amplitudes and phases, each accepted only after the exact 1-RDM identity verifies it; $v_A$ and $v_B$ are the flagship hand-checked instances (Theorems [1](#thm:psiA), [3](#thm:psiB)). The accompanying machine-readable bundle records, for every vertex, its classification and, where certified, the closed-form state (natural denominator, integer weights, symbolic amplitudes) together with its certificate. This is, to our knowledge, the first systematic library of exact extremal states for the generalized Pauli polytopes.
