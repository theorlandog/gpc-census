...
The proper test is specified but NOT YET RUN: construct the number field K
generated by all amplitude surds, compute its real embeddings, apply each
embedding consistently to the whole amplitude vector, and test attainment
exactly, with the expected orbit size pre-registered before step 3. It is
blocked on the v_B repair decision above, since the test should run on a
natural-orbital representative if one is to be shipped.
...
4. The v_B embedding test, properly constructed.
5. Whether the interference case admits any analogue of the design-case
