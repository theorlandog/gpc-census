# Research archive (superseded continuity notes)

> **ARCHIVED 2026-07.** This file is the frozen evidence log of the census
> program through the rank-10 closure. The living research document is now
> `docs/RESEARCH.md`, whose status tags point into the sections below by
> name; read that file first. Nothing here is deleted, but some sections are
> superseded snapshots. Known-stale figures, corrected in the living
> document, kept here as history: the "785 of 799" certified count (the
> shipped ledger is 799/799, CI-green); the "9 of 12 DESIGN-REAL at ratio 2"
> mining figure (the regenerated ledger ships all 12 DESIGN-REAL states at
> ratio 2). The P4 rigidity verdicts for v103 are annotated in place: they
> are fixed-rho statements, not fixed-spectrum statements (see
> docs/silence_rank_lemma.md).

This file encoded the working understanding of the research program so any
agent or collaborator could continue from the command line. House rules live
in AGENTS.md.
...
P4 **FAIL** -- kernel dim 5, no 1-term classes, registered law predicted
   ...
   [MERGE ANNOTATION 2026-07: "FIRST-ORDER RIGID" here is the FIXED-RHO
   verdict. Three of v103's five silent channels are cross-block and impose
   no first-order fixed-spectrum condition; the fixed-SPECTRUM fiber at v103
   is positive-dimensional (tangent dim 6, verified deformation curves).
   The lemma this paragraph asked for is proved in
   docs/silence_rank_lemma.md; the 5 = 5 coincidence is a fixed-rho fact.
   v89's rigidity stands in both senses.]
...
1-parameter family, v103 to a 14-determinant isolated point [MERGE
ANNOTATION 2026-07: isolated in that search's sparsity landscape only; the
v103 fixed-spectrum fiber is a 6-manifold and on-fiber sparsification
reaches support 12, see docs/unified_fiber_dimension.md]. Both certified
...
feasibility) [MERGE ANNOTATION 2026-07: a landscape statement about that
search, not a fiber statement; s_Q(v103) <= 12 via on-fiber sparsification
and the fixed-spectrum fiber germ is a smooth 6-manifold, see
docs/unified_fiber_dimension.md and docs/silence_rank_lemma.md]. Field over
primes 13,17: -5 sqrt(714)/221, -sqrt(170)/34,
...
- P4 (fiber dim = kernel dim for loopy supports w/o a rigid 1-term class): FAIL,
  ...
  "rigid 1-term class").
  [MERGE ANNOTATION 2026-07: "BOTH certified states are rigid (fiber dim 0)"
  is correct for v89 in both senses but for v103 only in the FIXED-RHO sense
  (all-channel silence rank 5 = kernel dim 5). At fixed SPECTRUM the three
  cross-block channels drop out and v103 is deformable: non-gauge tangent
  dim 6, with verified exact-spectrum deformation curves. See
  docs/silence_rank_lemma.md and the living RESEARCH.md; the P4 FAIL verdict
  itself is unaffected (the registered law predicted deformation dim 5).]
...
## PRE-REGISTRATION SCORED: v89 (parallel instance; independent corroboration)

NOTE (merge 2026-07): this is a SECOND, independent v89 pre-registration
scoring produced by the parallel instance. It agrees with the certified-state
scorecard in the section above (P3 and P4 FAIL; P5 PASS; P1 trivial-real;
P6 NOT-TESTED) and independently reproduces the exact v89 state (same
support-10, same squared weights (30,25,24,15,15,6,5,5,4,1)/130), which is
itself the required independent reproduction. Retained verbatim below.
...
that are NOT designs), and the tau = 0 rigidity class. v103 is now certified
and scored in the section above (P2 FAIL: five cancellation channels; P3 and
P4 FAIL; P5 PASS), resolving this pending note.
